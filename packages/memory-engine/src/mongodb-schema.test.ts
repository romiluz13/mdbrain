...
describe("ensureCollections", () => {
	...
	it("creates memory_evidence only when the evidence mirror is enabled", async () => {
		const previous = process.env.MDBRAIN_EVIDENCE_MIRROR_MODE
		process.env.MDBRAIN_EVIDENCE_MIRROR_MODE = "enabled"
		try {
			const db = mockDb([])
			await ensureCollections(db, "test_")
			expect(db.createCollection).toHaveBeenCalledWith(
			...
			if (previous === undefined) {
				delete process.env.MDBRAIN_EVIDENCE_MIRROR_MODE
			} else {
				process.env.MDBRAIN_EVIDENCE_MIRROR_MODE = previous
			}
		}
	})
...
describe("ensureStandardIndexes", () => {
	it("creates all standard indexes on chunks, embedding_cache, KB, and structured_mem", async () => {
		...
		// + 3 session_chunks + 1 uq_episodes_source_events = 86
...
describe("ensureSearchIndexes", () => {
	...
	it("creates only the session_chunks vector index for raw-session benchmark profile", async () => {
		const previousProfile = process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE
		const previousLane = process.env.MDBRAIN_BENCHMARK_RETRIEVAL_LANE
		process.env.MDBRAIN_BENCHMARK_RETRIEVAL_LANE = "raw-session"
		try {
			const db = mockDb()
			const result = await ensureSearchIndexes(
				db,
			...
			if (previousProfile === undefined) {
				delete process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE
			} else {
				process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE = previousProfile
			}
			if (previousLane === undefined) {
				delete process.env.MDBRAIN_BENCHMARK_RETRIEVAL_LANE
			} else {
				process.env.MDBRAIN_BENCHMARK_RETRIEVAL_LANE = previousLane
			}
		}
	})
	...
	it("does not set unsupported indexingMethod on autoEmbed vector indexes", async () => {
		const previousProfile = process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE
		process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE = "longmemeval"
		try {
			const db = mockDb()
	...
	it("includes session-aware token mappings in the chunks text index", async () => {
		...
		expect(textCall).toBeDefined()
		const textCallArg = textCall?.[0] as Document | undefined
		const textFields = textCallArg?.definition.mappings.fields
...
describe("search index readiness helpers", () => {
	...
	it("uses a smaller LongMemEval search-index target list when requested", () => {
		const previous = process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE
		process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE = "longmemeval"
		try {
			expect(
				getExpectedSearchIndexTargets("test_", "atlas-local-preview"),
			...
			if (previous === undefined) {
				...
				process.env.MDBRAIN_BENCHMARK_SEARCH_INDEX_PROFILE = previous
			}
		}
	})
	...
	it("resolves search index readiness timing from env with safe defaults", () => {
		...
		expect(
			resolveSearchIndexReadinessTiming({
				MDBRAIN_BENCHMARK_STRICT: "1",
			}),
		).toEqual({ timeoutMs: 180_000, pollMs: 1_000 })
		expect(
			resolveSearchIndexReadinessTiming({
				MDBRAIN_SEARCH_INDEX_READINESS_TIMEOUT_MS: "180000",
				MDBRAIN_SEARCH_INDEX_READINESS_POLL_MS: "250",
			}),
		).toEqual({ timeoutMs: 180_000, pollMs: 250 })
		expect(
			resolveSearchIndexReadinessTiming({
				MDBRAIN_SEARCH_INDEX_READINESS_TIMEOUT_MS: "0",
				MDBRAIN_SEARCH_INDEX_READINESS_POLL_MS: "nope",
			}),
...
describe("query_cache vector search index", () => {
	...
	it("creates memory_evidence Search and Vector Search indexes when enabled", async () => {
		...
		try {
			...
			const vectorCallArg = vectorCall?.[0] as Document | undefined
			const fields = vectorCallArg?.definition.fields ?? []
			const filterPaths = fields
