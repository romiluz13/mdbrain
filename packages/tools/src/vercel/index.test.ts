...
import { withMdbrain, _clearCache, type MdbrainCoreOptions } from "./index.js"
import type {
	LanguageModelV2,
...
describe("withMdbrain (Vercel AI SDK middleware)", () => {
	...
	it("uses wake-up mode by default when no user query is present", async () => {
		...
		const wrapped = withMdbrain(model, {
			...BASE_OPTIONS,
			mode: undefined,
		})
	...
	it("gracefully degrades when API returns 500", async () => {
		...
		// LLM call should succeed even when Mdbrain API is down
		const result = await wrapped.doGenerate(params)
		expect(result.content).toEqual([{ type: "text", text: "Hello from LLM" }])
